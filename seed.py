...
#Internal files
from model import connect_to_db, db, Country, User, Continent
...
def get_all_countries():
    ...
    r = requests.get("https://en.wikipedia.org/w/api.php?action=query&prop=revisions&format=json&rvprop=content&titles=List%20of%20sovereign%20states%20and%20dependent%20territories%20by%20continent")
    info = r.json()
    ...
    #             country = template.wikicode.get(1).value
    #             print "Country: "
    #             print country
    #         if template.get(5).value != None:
    #             defacto = template.get(5).value
    #             print "Defacto? "
    #             print defacto
    #         if template.get(6).value is not None:
    #             status = template.get(6).value
    #             print "Status?"
    #             print status
